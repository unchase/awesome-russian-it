...
|**Регулярность** ||
|**Длительность** ||
|**Количество эпизодов<br>(Apple Podcasts)**|<span itunes-id="" class="episodes"></span>|
...
- [ ] Добавьте дополнительные детали подкаста, такие как **`Количество эпизодов (Apple Podcasts)`** подкаста; **`Релиз последнего эпизода (Apple Podcasts)`** подкаста. Здесь во всех `<span>` атрибут **id** необходимо заполнить идентификатором подкаста в **Apple Podcasts** (например, *1065445951*, то есть без *id* в начале). Эта информация будет использоваться для автоматического ежедневного обновления данных по подкастам. 
